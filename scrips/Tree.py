...
#python Tree.py -a RF -w 10 -d ../datasets/UNI1.csv
parser = argparse.ArgumentParser(description='Trair Decition tree or Random forest.')
parser.add_argument('-a', '--algorithm', action='store', dest="algorithm", default="DT",
                    required=False, help="Input Pcap File.")
parser.add_argument('-d', '--dataset', action='store', dest="dataset", default="../datasets/UNI1.csv",
                    required=False, help="Window Time Off Flow.")
parser.add_argument('-w', '--weigth', action='store', dest="wt", default=10,
                    required=False, help="Window Time Off Flow.")
...
#Se ralizan las predicciones
predicciones = modelo.predict(X_test) #Predccion Total

mcc=matthews_corrcoef(y_test, predicciones)

pre0 = modelo.predict(dataT0) #Prediccion de 0
pre1 = modelo.predict(dataT1) #Prediccion de 1

#Se imprime el procentaje de Acierto de cada predccion
