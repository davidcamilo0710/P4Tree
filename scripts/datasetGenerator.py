...
parser = argparse.ArgumentParser(description='Process pcap and generate Dataset. NOTE: Only output first sub pcap.')
parser.add_argument('-i', '--input', action='store', dest="pcap_in", default="../PCAPs/UNI1.pcap", required=True, help="Input Pcap File.")
parser.add_argument('-w', '--window', action='store', dest="window_size", default=5.45, required=False, help="Window Time Off Flow.")
parser.add_argument('-d', '--outdir', action='store', dest="dir_out", default="../datasets", required=False, help="Output Directory.")
parser.add_argument('-l', '--log', action='store', dest="pcap_out", default="log", required=False, help="Output log File.")
parser.add_argument('-c', '--csv', action='store', dest="csv_out", default="dataset", required=False, help="Output dataset .csv File.")
...
for packet, (sec, usec, wirelen, caplen) in RawPcapReader(pcap_input):
    ...
    try:
        ...
        if ether_packet[IP]:
            ...
            ip_packet = ether_packet[IP]       # <<<< Assuming Ethernet + IPv4 here
            ...
            five_tuple_key = (ipv4_dst, ipv4_src, dport, sport, protocol, pcap_statistics['n_subflows'][five_tuple_bandera])
            pkt_len = wirelen
            ...
            if pcap_statistics['5_tuple_pkt_cnt_table'][five_tuple_key] <= 7:
                pcap_statistics['packt'+str(pcap_statistics['5_tuple_pkt_cnt_table'][five_tuple_key])][five_tuple_key] = pkt_len
            ...
            if(pcap_statistics['5_tuple_flow_size_table'][five_tuple_key]>10000):
                if pcap_statistics['count__by_nelly'][five_tuple_key] == 0: pcap_statistics['count_nellys'] += 1
...
for five_tuple in five_tuple_sorted_pkt_cnt:
    ...
    if Nelly == 1: toCSV[count,:] = [five_tuple[0][4], five_tuple[0][3], five_tuple[0][2], pcap_statistics['packt1'][five_tuple[0]], pcap_statistics['packt2'][five_tuple[0]], pcap_statistics['packt3'][five_tuple[0]], pcap_statistics['packt4'][five_tuple[0]], pcap_statistics['packt5'][five_tuple[0]], pcap_statistics['packt6'][five_tuple[0]], pcap_statistics['packt7'][five_tuple[0]],flow_size,Elephant]
    if Nelly == 1: count += 1

    of.write(tplt2.format(five_tuple[0][1], five_tuple[0][0], five_tuple[0][3], five_tuple[0][2], five_tuple[0][4], pkt_cnt, flow_size, Nelly, Elephant))
of.write("==============================================================================================================\n")
of.close()


## Export dataset csv.
DF = pd.DataFrame(toCSV, columns = ["ip_proto","port_src","port_dst","size_pkt1","size_pkt2","size_pkt3","size_pkt4","size_pkt5","size_pkt6","size_pkt7","tot_size","Elephant"])
