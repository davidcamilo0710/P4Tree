...
def export_p4(decision_tree):
    ...
    def _add_leaf(value, class_name, indent, prevfeature, result, depth, previous_id):
        ...
        print("table_add MyIngress.level", depth, " ", "MyIngress.SetClass", " ", previous_id, " ",
        ...
        if depth <= max_depth+1:
            ...
            if tree_.feature[node] != _tree.TREE_UNDEFINED:
                ...
                if int(name) == 9 or int(name) == 11:
                    ...
                    print("table_add MyIngress.level", depth, " MyIngress.CheckFeature ", previous_id, " ", prevfeature,
                          " ", result, " ", "=>", " ", current_id, " ", name, " ", hex(int(float(threshold))), sep='')
                ...
                print_tree_recurse(
                    tree_.children_left[node], depth+1, name, 1, current_id)
           ...
           #     print("level", depth, "checkfeature", prevfeature, result, "=>", name, threshold)
                ...
                print_tree_recurse(
                    tree_.children_right[node], depth+1, name, 0, current_id)
            else:  # leaf
                _add_leaf(value, class_name, indent, prevfeature,
                          result, depth, previous_id)

    print_tree_recurse(0, 1, 0, 1, global_id)


print("Tree in commands_DT.txt")
sys.stdout = open("commands_DT.txt", "w")
export_p4(rf)
sys.stdout.close()
